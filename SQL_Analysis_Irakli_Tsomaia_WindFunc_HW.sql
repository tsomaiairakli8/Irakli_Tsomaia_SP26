...
FROM (
    -- subquery to get sold_amount for every customer/channel/year
    SELECT 
        ...
        RANK() OVER (PARTITION BY times.calendar_year, channels.channel_id ORDER BY SUM(sales.amount_sold) DESC) AS cust_rank -- rank customers by amount_sold
    FROM sh.sales
    ...
    GROUP BY customers.cust_id, channels.channel_id, times.calendar_year
) tab
WHERE cust_rank <= 300 -- filter only top 300 customers
GROUP BY cust_id, channel_desc, cust_last_name, cust_first_name
HAVING COUNT(DISTINCT calendar_year) = 3 -- customers which are in top 300 among all years
