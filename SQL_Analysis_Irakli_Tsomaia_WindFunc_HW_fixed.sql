...
-- Solution logic: We use cte to filter the top 300 customers separately and we join the cte to sales table.
WITH top_1998 AS ( -- Top 300 in 1998
    SELECT s.cust_id
    FROM sh.sales s
...
-- Combining the three distinct top lists into one pool
target_customers AS (
    SELECT cust_id FROM top_1998
    UNION
    SELECT cust_id FROM top_1999
    UNION
    SELECT cust_id FROM top_2001
)
...
JOIN sh.times t ON s.time_id = t.time_id
-- Filter the entire sales dataset by our pool of top performers
JOIN target_customers tc ON s.cust_id = tc.cust_id
