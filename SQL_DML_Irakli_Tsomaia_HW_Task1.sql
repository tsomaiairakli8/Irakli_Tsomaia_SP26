...
/*Task find person with more than 43 rental and payment records and alter their name to yours. FIXED no hardcoding, CURRENT_DATE is removed
...
SET 
    ...
    first_name = 'IRAKLI', 
    last_name = 'TSOMAIA', 
    email = 'IRAKLI.TSOMAIA@sakilacustomer.org', 
    address_id = 604, 
    activebool = true,  
    last_update = now(), 
    active = 1
WHERE customer_id = (
    ...
    FROM (
        SELECT customer_id FROM public.rental
    ...
    INNER JOIN (
        ...
        GROUP BY customer_id
        HAVING COUNT(*) > 43
    ) p ON r.customer_id = p.customer_id
...
WHERE customer_id = (SELECT customer_id FROM public.customer WHERE first_name = 'IRAKLI' AND last_name = 'TSOMAIA' LIMIT 1);
RETURNING *;
COMMIT;
...
BEGIN;
CREATE TABLE IF NOT EXISTS public.payment_default PARTITION OF public.payment DEFAULT; -- FIXED 
COMMIT;
...
CURRENT_TIMESTAMP, 
    i.inventory_id, 
    (
        -- Subquery replaces the hard-coded 598
        WITH cte_rentals AS (
            SELECT customer_id FROM public.rental
            GROUP BY customer_id
            HAVING COUNT(*) > 43 
...
WHERE r.customer_id = (
    ...
    INNER JOIN (
        ...
        GROUP BY customer_id 
        HAVING COUNT(*) > 43
    ) pay ON rent.customer_id = pay.customer_id
    ORDER BY rent.customer_id DESC 
    LIMIT 1
)
RETURNING *;
COMMIT;
